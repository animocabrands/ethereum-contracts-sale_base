...
describe('SwapSale', function () {
  ...
  async function doDeploy(params = {}) {
    this.referenceToken = await ERC20.new(params.referenceTokenSupply || ether('1000'), {from: owner});

    this.erc20Token = await ERC20.new(params.erc20TokenSupply || ether('1000'), {from: owner});
  ...
  describe('swapRates()', function () {
    ...
    it('should revert if the oracle does not provide a swap rate for one of the pairs', async function () {
      await expectRevert(this.contract.swapRates([this.ethTokenAddress], referenceTokenPrice, userData), 'SwapSaleMock: undefined swap rate');
    });
  ...
  describe('_oraclePricing()', function () {
    ...
    it('should revert if the payment token is not supported by the SKU', async function () {
      const otherToken = await ERC20.new(ether('1000'), {from: owner});
    ...
    it('should set the oracle total price (1 == rate)', async function () {
      ...
      const token = await ERC20.new(ether('1000'), {from: owner});
