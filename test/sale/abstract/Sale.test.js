...
describe('Sale', function () {
  ...
  async function doUpdateSkuPricing(params = {}) {
    ...
    return await this.contract.updateSkuPricing(params.sku || sku, [this.tokenAddress], [this.tokenPrice], {from: params.owner || owner});
  }
  ...
  describe('start()', function () {
    ...
    it('reverts if called by any other than the contract owner', async function () {
      await expectRevert(doStart.bind(this)({owner: purchaser}), 'Ownable: caller is not the owner');
    });

    it('reverts if the contract has already started', async function () {
      await doStart.bind(this)();
      await expectRevert(doStart.bind(this)(), 'Startable: started');
    });

    it('reverts if the contract is not paused', async function () {
      await this.contract.setPaused(false);
      await expectRevert(doStart.bind(this)(), 'Pausable: not paused');
    });
  ...
  describe('pause()', function () {
    ...
    it('reverts if called by any other than the contract owner', async function () {
      await expectRevert(this.contract.pause({from: purchaser}), 'Ownable: caller is not the owner');
    });

    it('reverts if the contract has not been started yet', async function () {
      await expectRevert(this.contract.pause({from: owner}), 'Startable: not started');
    });
  ...
  describe('unpause()', function () {
    ...
    it('reverts if called by any other than the contract owner', async function () {
      await doStart.bind(this)();
      await this.contract.pause({from: owner});
      await expectRevert(this.contract.unpause({from: purchaser}), 'Ownable: caller is not the owner');
    });

    it('reverts if the contract has not been started yet', async function () {
      await expectRevert(this.contract.unpause({from: owner}), 'Startable: not started');
    });
  ...
  describe('updateSkuPricing()', function () {
    ...
    it('reverts if called by any other than the contract owner', async function () {
      await expectRevert(doUpdateSkuPricing.bind(this)({owner: purchaser}), 'Ownable: caller is not the owner');
    });

    it('reverts if `tokens` and `prices` have different lengths', async function () {
      const ethTokenAddress = await this.contract.TOKEN_ETH();

      await expectRevert(this.contract.updateSkuPricing(sku, [ethTokenAddress], [], {from: owner}), 'Sale: tokens/prices lengths mismatch');
    });

    it('reverts if `sku` does not exist', async function () {
      await expectRevert(doUpdateSkuPricing.bind(this)({sku: stringToBytes32('otherSku')}), 'Sale: non-existent sku');
    });
    ...
    it('should emit the SkuPricingUpdate event', async function () {
      ...
      expectEvent(receipt, 'SkuPricingUpdate', {
        sku: sku,
  ...
  describe('purchaseFor()', function () {
    ...
    it('reverts if the sale has not started', async function () {
      const quantity = One;
      await expectRevert(
        this.contract.purchaseFor(recipient, this.ethTokenAddress, sku, quantity, userData, {
          from: purchaser,
    ...
    it('reverts if `recipient` is the zero address', async function () {
      ...
      const quantity = One;
      await expectRevert(
        this.contract.purchaseFor(ZeroAddress, this.ethTokenAddress, sku, quantity, userData, {
          from: purchaser,
          value: ethPrice.mul(quantity),
        }),
    ...
    it('reverts if `token` is the zero address', async function () {
      ...
      const quantity = One;
      await expectRevert(
        this.contract.purchaseFor(recipient, ZeroAddress, sku, quantity, userData, {
          from: purchaser,
          value: ethPrice.mul(quantity),
        }),
    ...
    it('reverts if `sku` does not exist', async function () {
      ...
      const quantity = One;
      await expectRevert(
        this.contract.purchaseFor(recipient, this.ethTokenAddress, stringToBytes32('otherSku'), quantity, userData, {
          from: purchaser,
          value: ethPrice.mul(quantity),
        }),
    ...
    it('reverts if `quantity` is greater than the remaining supply', async function () {
      ...
      await this.contract.purchaseFor(recipient, this.ethTokenAddress, sku, quantity, userData, {
        from: purchaser,
        value: ethPrice.mul(quantity),
      });
      await expectRevert(
    ...
    it('reverts if `sku` exists but does not have a price set for `token`', async function () {
      ...
      const quantity = One;
      await expectRevert(
        this.contract.purchaseFor(recipient, erc20Token.address, sku, quantity, userData, {from: purchaser}),
    ...
    it('should perform a purchase', async function () {
      await doStart.bind(this)();
      const quantity = One;
      const receipt = await this.contract.purchaseFor(recipient, this.ethTokenAddress, sku, quantity, userData, {
        from: purchaser,
        value: ethPrice.mul(quantity),
      });
  ...
  describe('estimatePurchase()', function () {
    ...
    it('reverts if the sale has not started', async function () {
      const quantity = One;
      await expectRevert(
        this.contract.estimatePurchase(recipient, this.ethTokenAddress, sku, quantity, userData, {from: purchaser}),
        'Startable: not started'
      );
    });
    ...
    it('reverts if `recipient` is the zero address', async function () {
      ...
      const quantity = One;
      await expectRevert(
        this.contract.estimatePurchase(ZeroAddress, this.ethTokenAddress, sku, quantity, userData, {from: purchaser}),
        'Sale: zero address recipient'
      );
    });
    ...
    it('reverts if `token` is the zero address', async function () {
      ...
      const quantity = One;
      await expectRevert(
        this.contract.estimatePurchase(recipient, ZeroAddress, sku, quantity, userData, {from: purchaser}),
        'Sale: zero address token'
      );
    });
    ...
    it('reverts if `sku` does not exist', async function () {
      ...
      const quantity = One;
      await expectRevert(
        this.contract.estimatePurchase(recipient, this.ethTokenAddress, stringToBytes32('otherSku'), quantity, userData, {from: purchaser}),
        'Sale: non-existent sku'
      );
    });
    ...
    it('reverts if `sku` exists but does not have a price set for `token`', async function () {
      ...
      const quantity = One;
      await expectRevert(
        this.contract.estimatePurchase(recipient, erc20Token.address, sku, quantity, userData, {from: purchaser}),
        'Sale: non-existent sku token'
      );
    });
    ...
    it('should estimate the computed final total amount to pay', async function () {
      await doStart.bind(this)();
      const quantity = One;
      const priceInfo = await this.contract.estimatePurchase(recipient, this.ethTokenAddress, sku, quantity, userData, {from: purchaser});
  ...
  describe('getSkuInfo()', function () {
    ...
    it('reverts if `sku` does not exist', async function () {
      await expectRevert(this.contract.getSkuInfo(stringToBytes32('otherSku')), 'Sale: non-existent sku');
    });
  ...
  describe('_createSku()', function () {
    ...
    it('reverts if `totalSupply` is zero', async function () {
      await expectRevert(doCreateSku.bind(this)({skuTotalSupply: Zero}), 'Sale: zero supply');
    });

    it('reverts if `sku` already exists', async function () {
      await doCreateSku.bind(this)();
      await expectRevert(doCreateSku.bind(this)(), 'Sale: sku already created');
    });
    ...
    it('reverts if the update results in too many SKUs', async function () {
      ...
      await expectRevert(doCreateSku.bind(this)({sku: stringToBytes32('anotherSku')}), 'Sale: too many skus');
    });
  ...
  describe('_setTokenPrices()', function () {
    ...
    it('reverts if one of the `tokens` is the zero address', async function () {
      await expectRevert(this.contract.setTokenPrices(sku, [ZeroAddress], [ethPrice]), 'Sale: zero address token');
    });
    ...
    it('reverts if the sku token price set exceeds the defined tokens per-sku capacity', async function () {
      ...
      await expectRevert(
        this.contract.setTokenPrices(sku, [erc20Token1.address, erc20Token2.address, erc20Token3.address], [ethPrice, ethPrice, ethPrice]),
    ...
    it('should disable a SKU token if provided a zero price', async function () {
      ...
      await this.contract.setTokenPrices(sku, [this.ethTokenAddress], [Zero]);
      const skuInfoAfter = await this.contract.getSkuInfo(sku);
    ...
    it('should add support for a SKU token if provided a price for an unsupported token', async function () {
      ...
      await this.contract.setTokenPrices(sku, [this.ethTokenAddress], [ethPrice]);
      const skuInfoAfter = await this.contract.getSkuInfo(sku);
      skuInfoAfter.tokens.length.should.equal(1);
      skuInfoAfter.prices.length.should.equal(1);
    ...
    it('should update a SKU token price', async function () {
      ...
      await this.contract.setTokenPrices(sku, [this.ethTokenAddress], [updatedPrice]);
      const skuInfoAfter = await this.contract.getSkuInfo(sku);
      skuInfoAfter.prices.length.should.equal(1);
  ...
  describe('_validation()', function () {
    ...
    it('reverts if `purchase.recipient` is the zero address', async function () {
      await expectRevert(
        this.contract.validation(ZeroAddress, this.ethTokenAddress, sku, One, userData, Zero, [], [], []),
        'Sale: zero address recipient'
      );
    });

    it('reverts if `purchase.token` is the zero address', async function () {
      await expectRevert(this.contract.validation(recipient, ZeroAddress, sku, One, userData, Zero, [], [], []), 'Sale: zero address token');
    });

    it('reverts if `purchase.quantity` is zero', async function () {
      await expectRevert(
        this.contract.validation(recipient, this.ethTokenAddress, sku, Zero, userData, Zero, [], [], []),
        'Sale: zero quantity purchase'
      );
    });
    ...
    it('reverts if `purchase.sku` does not exist', async function () {
      await expectRevert(
        this.contract.validation(recipient, this.ethTokenAddress, stringToBytes32('otherSku'), One, userData, Zero, [], [], []),
        'Sale: non-existent sku'
      );
    });
    ...
    it('reverts if `purchase.quantity` is greater than the maximum purchase quantity', async function () {
      await expectRevert(
        this.contract.validation(recipient, this.ethTokenAddress, sku, skuMaxQuantityPerPurchase.addn(1), userData, Zero, [], [], []),
        'Sale: above max quantity'
      );
    });
    ...
    it('reverts if `purchase.quantity` is greater than the remaining supply', async function () {
      ...
      await expectRevert(
        this.contract.validation(recipient, this.ethTokenAddress, sku, quantity, userData, Zero, [], [], []),
        'Sale: insufficient supply'
      );
    });
    ...
    it('reverts if `purchase.sku` exists but does not have a price set for `purchase.token`', async function () {
      const erc20Token = await ERC20.new(erc20TotalSupply, {from: owner});
      await expectRevert(
        this.contract.validation(recipient, erc20Token.address, sku, One, userData, Zero, [], [], []),
        'Sale: non-existent sku token'
      );
    });
  });
  ...
  describe('_delivery()', function () {
    ...
    it('should not update the SKU total supply if the supply is unlimited', async function () {
      ...
      await this.contract.delivery(ZeroAddress, ZeroAddress, sku, One, userData, Zero, [], [], []);
      const skuInfoAfter = await this.contract.getSkuInfo(sku);
  ...
  describe('_notification()', function () {
    ...
    it('should emit the Purchase event', async function () {
      ...
      expectEvent(receipt, 'Purchase', {
        ...
        userData: userData,
        totalPrice: ethPrice,
        extData: bytes32ArraysToBytesPacked([pricingData, paymentData, deliveryData]),
      });
    });

    it("should call the SKU's notification receiver if defined", async function () {
      const receipt = await this.contract.notification(recipient, this.ethTokenAddress, sku, One, userData, ethPrice, [], [], [], {from: purchaser});

      await expectEvent.inTransaction(receipt.tx, this.purchaseNotificationsReceiver, 'PurchaseNotificationReceived', {});
    });
  });
});
