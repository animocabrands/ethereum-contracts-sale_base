...
describe('PurchaseNotificationsReceiver', function () {
  ...
  describe('onPurchaseNotificationReceived()', function () {
    ...
    it('should emit the PurchaseNotificationReceived event', async function () {
      ...
      expectEvent(receipt, 'PurchaseNotificationReceived', {});
    });

    it('should return the sha3 hash of the function selector', async function () {
      await this.contract.onPurchaseNotificationReceived(purchaser, recipient, ZeroAddress, sku, One, userData, ethPrice, [], [], [], {
        from: purchaser,
      });

      const events = await this.contract.getPastEvents('OnPurchaseNotificationReceivedResult', {fromBlock: 'latest'});
